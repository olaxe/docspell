...
RUN ["/bin/bash", "-c", "sed -n -e '/full-text-search/,/^  }/ p' /opt/docspell/restserver/conf/docspell-server.conf | sed -e '/enabled/ s/=.*/= \$$\{DOCSPELL_FULL_TEXT_SEARCH_ENABLED\}/' >/tmp/__full_text_search"]
RUN ["/bin/bash", "-c", "rg --replace '`cat /tmp/__full_text_search`' --passthru --no-line-number --multiline --multiline-dotall '  full-text-search.*?\n  }\n' /opt/docspell/restserver/conf/docspell-server.conf \
    > /opt/docspell/restserver/conf/docspell-server.conf"]
